...
/*
 *   STUDENT NAME      :  Byron.Chen
 *   STUDENT ID        :  260892558
 *   
...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	//  initialize the result as an empty list of coefficients
			...
			if (this.compareTo(divisor) < 0){
				System.out.println("Error: a.dividedBy(b) requires that a >= b");
				throw new Exception();
			}
			
			quotient = this.partDivision(divisor);
			
	
			// Remove leading zeros
			while (quotient.coefficients.size() > 0 && quotient.coefficients.get(quotient.coefficients.size()-1) == 0)
				quotient.coefficients.remove(quotient.coefficients.size()-1);

		return quotient;		
	}
	
	public MyBigInteger partDivision(MyBigInteger d) throws Exception {
		MyBigInteger temp_D = this.clone();
		MyBigInteger result = new MyBigInteger( this.base );
		MyBigInteger r = new MyBigInteger( this.base );
		int partSize = d.coefficients.size();
		
		for(int i = 0; i < this.coefficients.size() - d.coefficients.size() + 1; i++) {						// times the part division needed to be activated
			MyBigInteger D_part = new MyBigInteger(this.base);												// the part of D
			
			for (int j = temp_D.coefficients.size() - partSize; j < temp_D.coefficients.size(); j++) {		// to get part of the numbers in divided(D) with same size of divisor(d)
				D_part.coefficients.add( Integer.valueOf(temp_D.coefficients.get(j)) );
			}

			
			if(D_part.compareTo(d) != -1) {																//do the division only when D_part is bigger than d
				result.coefficients.add(0, Integer.valueOf(D_part.slowdividedBy(d).coefficients.get(0)));	//get the quotient of this part division, the largest times operated are 9
				MyBigInteger q = new MyBigInteger(result.coefficients.get(0),this.base);
				
				r = D_part.minus(d.times(q));																		//get the remainder of this part division
				while(r.coefficients.size() < D_part.coefficients.size()) {								//let the remainder have the same size as the D_part
					r.coefficients.add(Integer.valueOf(0));
				}
				
				int position = temp_D.coefficients.size() - 1;
				for (int k = r.coefficients.size() - 1; k >= 0; k--) {					//refresh the D_temp by replacing them with the remainder
					temp_D.coefficients.set(position, Integer.valueOf(r.coefficients.get(k)));
					position--;
				}
			}else {
				result.coefficients.add(0,Integer.valueOf(0));
...
//		MyBigInteger remainder = this.clone();   
		...
		if (newBase < 2 || newBase > 10) {
			...
			if (this.base > newBase) {
				result = highToLow(newBase);
			}else if (this.base < newBase) {
				result = lowToHigh(newBase);
			}
		}

//		}else {
//		     int num_result = toNumber(remainder.toDecimal());
//		     int b;
//		     System.out.println("num_result: " + num_result);
//	    	 
//		     while (num_result > 0) {
//		    	 b = num_result % newBase;
//		    	 num_result = num_result / newBase;
//		    	 result.coefficients.add(Integer.valueOf(b));
//		     }
//		}
//        
//		
	...
	public MyBigInteger highToLow(int newBase) throws Exception {
		MyBigInteger remainder = this.clone(); 
		MyBigInteger nb = new MyBigInteger(newBase, this.base);
		MyBigInteger result = new MyBigInteger(newBase);
		MyBigInteger comparition = new MyBigInteger(0, this.base);
		
		while(remainder.compareTo(comparition) == 1) {		// when the number is not 0
			
			result.coefficients.add(Integer.valueOf(remainder.mod(nb).coefficients.get(0)));
			
			if (remainder.compareTo(nb) == -1) break;
			
			remainder = remainder.dividedBy(nb);
		}
		return result;
	}
	...
	public MyBigInteger lowToHigh(int newBase) throws Exception {
		MyBigInteger remainder = this.clone(); 
		MyBigInteger nb = new MyBigInteger(this.base, newBase);
		MyBigInteger mult = new MyBigInteger(1, newBase);
		MyBigInteger result = new MyBigInteger(newBase);
		
		for(int i = 0; i < remainder.coefficients.size(); i++) {		
			MyBigInteger tmp = new MyBigInteger(newBase);
			
			tmp.coefficients.add(Integer.valueOf(remainder.coefficients.get(i)));
			tmp.timesBaseToThePower(i);
			result = result.plus(tmp.times(mult));
			mult = mult.times(nb);
		}
		return result;
	}
...
//			
//			for(int i = 0; i < remainder.coefficients.size(); i++) {
//				e = remainder.coefficients.get(i) * (int)Math.pow(b,i);
//				e = remainder.coefficients.get(i) % 10 + carry;
//				carry = e / 10;
//				result.coefficients.add(Integer.valueOf(e));
//			}
//			return result;
//		}
//	}
//	
//	public int toNumber(MyBigInteger Num) {
//		int n = 0;
//		int tmp = 0;
//		int power = Num.coefficients.size() - 1;
//		
//		for (int i = Num.coefficients.size() - 1; i >= 0; i--) {
//			tmp = (int) (Num.coefficients.get(i) * Math.pow(base, power));			//????????????????????????????????
//			n += tmp;
//			power--;
//		}
//		return n;
...
//	
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		MyBigInteger remainder = this.clone();
		MyBigInteger potentialFactor = new MyBigInteger (2, this.base);
		MyBigInteger one = new MyBigInteger (1, this.base);
		MyBigInteger two = new MyBigInteger (2, this.base);
		MyBigInteger squareRoot = this.findSquareRoot();
		
		while(remainder.compareTo(one) > 0 && potentialFactor.compareTo(squareRoot) < 0) {
			if (remainder.isFactor(potentialFactor) && potentialFactor.isPrime()) {
				factors.add(potentialFactor);
				remainder = remainder.dividedBy(potentialFactor);
			}else {
				if(potentialFactor.compareTo(two) == 0) {
					potentialFactor = potentialFactor.plus(one);
				}else {
					potentialFactor = potentialFactor.plus(two);
				}
			}
		}
...
//		ArrayList<MyBigInteger> factors = new ArrayList<MyBigInteger>();
//		MyBigInteger remainder = this.clone();
//		MyBigInteger potentialFactor = this.findSquareRoot();
//		MyBigInteger two = new MyBigInteger (2, this.base);
//		
//		while(remainder.compareTo(two) > -1) {
//			if (remainder.compareTo(potentialFactor) >= 0 && remainder.isFactor(potentialFactor)){
//				if(potentialFactor.isPrime()){
//					factors.add(0, potentialFactor);
//					remainder = remainder.dividedBy(potentialFactor);
//				}
//			}else {
//				potentialFactor = potentialFactor.minus(two);
//				if (potentialFactor.compareTo(two) == -1) potentialFactor = two;
//			}
//		}
		
		if (factors.size() == 0) factors.add(remainder);
		return factors;
	}
	
	public boolean isPrime() throws Exception {
		MyBigInteger potentialFactor = new MyBigInteger (3, this.base);
		MyBigInteger one = new MyBigInteger (1, this.base);
		MyBigInteger two = new MyBigInteger (2, this.base);
		MyBigInteger stopPoint = this.dividedBy(two).plus(one);		//stopPoint = this/2 + 1
		MyBigInteger increment = new MyBigInteger (2,this.base);
		
		if (this.compareTo(two) == 0) return true;
		
		while (this.compareTo(stopPoint) != 1) {
			if (this.mod(two).compareTo(one) != 0 || this.isFactor(potentialFactor)) {
				return false;
			}else {
				potentialFactor = potentialFactor.plus(increment);
			}
		}
		return true;
	}
	
	public boolean isFactor(MyBigInteger num) throws Exception {
		boolean flag = false;
		MyBigInteger zero = new MyBigInteger (0, this.base);
		
		if (this.mod(num).compareTo(zero) == 0) flag = true;
		
		return flag;
	}
	
	public MyBigInteger findSquareRoot() throws Exception {
		MyBigInteger temp = new MyBigInteger (this.base);
		MyBigInteger two = new MyBigInteger (2, this.base);
		MyBigInteger one = new MyBigInteger (1, this.base);
		MyBigInteger zero = new MyBigInteger (0, this.base);
		MyBigInteger sr = this.dividedBy(two);
		
		while(true) {
			temp = sr;
			sr = temp.plus(this.dividedBy(temp)).dividedBy(two);		// sr = (temp + (number / temp)) / 2;
			if(temp.compareTo(sr) == 0) break;
		}
		if(sr.mod(two).compareTo(zero) == 0) return sr.plus(one);
		else return sr.plus(two);
	}
	
	
	
//	public MyBigInteger findSquareRoot() throws Exception {
//		MyBigInteger increment = new MyBigInteger (1,this.base);
//		MyBigInteger i = new MyBigInteger (1,this.base);
//		
//		while(true) {
//			if(this.isSquareRoot(i)) return i;
//			i = i.plus(increment);
//		}
//	}
//	
//	public boolean isSquareRoot(MyBigInteger n) throws Exception {
//		if(this.compareTo(n.times(n)) <= 0) return true;
//		return false;
//	}
	...
	*/
	...
	public MyBigInteger mod( MyBigInteger  divisor ) throws Exception{
		...
		if(this.compareTo(divisor) == -1) return this;
	...
	public int compareTo(MyBigInteger second) {
		...
		// Remove leading zeros
		while (second.coefficients.size() > 0 && second.coefficients.get(second.coefficients.size()-1) == 0)
			second.coefficients.remove(second.coefficients.size()-1);
					
		while (remainder.coefficients.size() > 0 && remainder.coefficients.get(remainder.coefficients.size()-1) == 0)
		...
		int diff = remainder.coefficients.size() - second.coefficients.size();
		if (diff < 0)
			return -1;
		else if (diff > 0)
			return 1;
			...
			boolean done = false;
			int i = remainder.coefficients.size() - 1;
			while (i >=0 && !done){
				diff = remainder.coefficients.get(i) - second.coefficients.get(i); 
				if (diff < 0){
					return -1;
				}
				else if (diff > 0)
